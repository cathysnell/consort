...
function setup(): { consortDir: string; designLog: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "lbscm-recon2-"));
  tmps.push(root);
  const consortDir = path.join(root, ".sftdd");
  fs.mkdirSync(consortDir, { recursive: true });
  return { consortDir, designLog: path.join(root, "agent-log.design.jsonl") };
}
...
describe("reconstituteAgentLog", () => {
  it("uses recorded design entries verbatim (date + cost), drops reconciled, re-dates live build onto the design timeline", () => {
    const { consortDir, designLog } = setup();
    ...
    const final = reconstituteAgentLog({ consortDir, designLogPath: designLog });
