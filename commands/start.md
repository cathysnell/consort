...
2. **Continue the loop.** Offer the human the autonomous path or a single step:
   ...
   - Or one phase at a time (lowest-ready first):
     - No sprint backlog (or the last sprint shipped):
       - **Intake FIRST (no-seed projects) – open the Product Owner interview.** `/plan` and `/design` REQUIRE the project intake on disk: `.consort/product-overview.md`, `.consort/nfrs.md`, and (UI track only) `.consort/design/design-brief.md`. The **StockFlow seed supplies these; a project the user authored themselves starts WITHOUT them**, and the Spec Author cannot propose from nothing. So when they are ABSENT, run the **Product Owner intake interview BEFORE `/plan`**: invoke the **`product-owner` agent** (`.claude/agents/product-owner.md`, its facilitator role – "drafts FOR the human; the human approves; never invents intent") and follow its intake Method – three short interviews, drafting each artifact for the human's approval:
         1. **Product → `.consort/product-overview.md`** – who it's for, its purpose, how it grows, what to see after each sprint (H1 + body, no implementation detail).
