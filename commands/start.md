...
2. **Continue the loop — do (a) → (b) → (c) → (d) IN ORDER. Do not collapse them, do not reorder them, and do not bundle drive-mode or UI-track into an early "quick decisions" prompt.**

   **(a) Offer the live dashboard — now, as its own step.** Present it and let the human act on it; do NOT reduce it to a passive "ask me anytime" aside. `./scripts/run-dashboard.sh` reads THIS project's `.consort/` off disk (no git, no remote) and opens the browser radiator (roles, gates, per-turn transcripts, test counts); it runs in its own terminal. Offer it regardless of where you're running.

   **(b) Editor — ONLY if you are NOT already in one.** If the session is not in an editor's integrated terminal, suggest opening the project in VS Code / Cursor (`code "$PWD"`) so per-turn artifact auto-open works. **If you ARE already in the IDE terminal (`$TERM_PROGRAM` is `vscode`/`cursor`, or `$VSCODE_*` is set), say NOTHING about opening VS Code or installing the extension — you are already in the right place. Do not offer the extension unless the human asks.**

   **(c) Intake — capture the PRODUCT next (no-seed projects).** **HARD RULE: do NOT present a "how do you want to drive" (step-by-step vs sprint) question, and do NOT present a "UI track" question, before the product interview. Asking either before the interview is a defect.** `/plan`/`/design` REQUIRE `.consort/product-overview.md`, `.consort/nfrs.md`, and (UI only) `.consort/design/design-brief.md`; a self-authored project starts WITHOUT them (product-overview.md is still the empty template). SKIP step (c) entirely when the artifacts already exist (seed path / resumed project). When ABSENT:
   ...
   **(d) THEN — and only now (product captured, or a seed/resumed project) — offer how to drive:**
