   ...
   - Or one phase at a time (lowest-ready first):
     - No sprint backlog (or the last sprint shipped):
       - **Intake FIRST (no-seed projects) – interview, then the metered PO turn drafts.** `/plan` and `/design` REQUIRE the project intake on disk: `.consort/product-overview.md`, `.consort/nfrs.md`, and (UI track only) `.consort/design/design-brief.md`. The **StockFlow seed supplies these; a project the user authored themselves starts WITHOUT them**, and the Spec Author cannot propose from nothing. So when they are ABSENT, do TWO things before `/plan` (**SKIP both when the artifacts already exist** – the seed path, or a resumed project):
         1. **Interview the human – this is you, the coordinating session (the live, conversational part).** Ask the intake questions and capture the answers to **`.consort/intake/answers.md`**. Ground the questions in the canon so they aren't improvised: **`@software-design-principles`** for the NFRs (walk performance / scalability / security / observability / operability / resilience) and **`@ui-ux-design-principles`** for the product framing + (UI track) the design brief (reference sites, brand / interaction / accessibility). Keep it short; the answers file is the PO turn's input, not the final artifact.
