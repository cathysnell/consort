...
**Step 1 — offer the live dashboard, as its OWN step.** Regardless of terminal or editor, present **`./scripts/run-dashboard.sh`** — it opens the browser radiator (roles, gates, per-turn transcripts, test counts) reading THIS project's `.consort/` off disk (no git), runs in its own terminal, and works from tmux or an IDE alike. Present it and let the human start it (or decline) BEFORE Step 2. **Do NOT bury it as an aside inside the editor question — it is a standalone offer, given every time, whether or not they're in an editor.**
