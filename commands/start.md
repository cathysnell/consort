...
2. **Continue the loop.** **FIRST — before you invoke ANY command (the intake interview, `/plan`, `/sprint`, `/design`, …) — surface these two ways to work the run and let the human act. This is a REQUIRED checkpoint: present it and pause; do NOT skip straight into the interview or a phase. (They are offers — the drive behaves the same either way — but you must offer them.)**
   - **Open the dashboard to watch it live** — `./scripts/run-dashboard.sh`. It reads THIS project's local `.consort/` off disk (no git, no remote) and opens the browser radiator (roles, gates, per-turn transcripts, test counts). Uses the locally-deployed kit's prebuilt dashboard (dev-clone kits fall back to a dev server); it runs in the foreground, so start it in its own terminal.
   - **Open the project in VS Code to run the drive there** — `code "$PWD"` (or the user's editor). Running the commands from the editor's integrated terminal is what lets per-turn artifact auto-open work; the Consort extension surfaces the same commands. (Already in VS Code? this half is satisfied — still offer the dashboard.)

   Then offer the human the autonomous path or a single step:
   ...
   - Or one phase at a time (lowest-ready first):
     - No sprint backlog (or the last sprint shipped):
       - **Intake FIRST (no-seed projects) – interview, then the metered PO turn drafts.** `/plan` and `/design` REQUIRE the project intake on disk: `.consort/product-overview.md`, `.consort/nfrs.md`, and (UI track only) `.consort/design/design-brief.md`. The **StockFlow seed supplies these; a project the user authored themselves starts WITHOUT them**, and the Spec Author cannot propose from nothing. So when they are ABSENT, do TWO things before `/plan` (**SKIP both when the artifacts already exist** – the seed path, or a resumed project):
         1. **Interview the human – this is you, the coordinating session (the live, conversational part).** Conduct the intake **wizard-style per `skills/consort/references/hil-interview.md`**: open with the domain + project name in one or two sentences, then walk the rest ONE QUESTION AT A TIME (product overview → NFRs → UI design brief), grounding the question content in the canon (**`@software-design-principles`** for the NFR categories, **`@ui-ux-design-principles`** for the product framing + design brief) so it is never improvised or free-form. Copy `intake-answers-template.md` to **`.consort/intake/answers.md`** and fill each section as you go. Keep it short; the answers file is the PO turn's input, not the final artifact.
            - **UI track — you MUST ask the reference-sites question; do NOT skip it.** For a UI project the design-brief part REQUIRES: **"Which real websites or apps should this look like? Give 1–3 URLs, and say what to take from each (layout, color, typography, density, a specific screen)."** This is load-bearing: the UX Designer literally OPENS those sites (Playwright) and models the design-guide tokens on them — a design brief with NO reference sites leaves every screen on the generic Databricks baseline (the "everything looks the same" failure). Suggest a couple of candidate sites for their domain to prompt them if they're unsure (you may WebSearch for examples), but capture THEIR choices; record them under the template's "Design brief" section. Skip this ONLY for a backend-only (`--no-ui-track`) project.
         2. **Let the metered Product Owner turn DRAFT the artifacts.** Then run **`/plan`** – the drive dispatches the **`product-owner` `intake` turn** (a real, tracked agent turn: model `opus` by default, `turn.usage` logs its tokens + cost, it shows in telemetry + the dashboard PO card, and the model is settable via `roles.product-owner`). It reads `intake/answers.md` + the canon + the **StockFlow worked example** (`examples/first-project/stockflow-seed/intake/`) and writes `.consort/product-overview.md`, `.consort/nfrs.md`, and (UI) `.consort/design/design-brief.md` – drafting FOR the human, never inventing beyond the stated answers. You then **review + approve** the drafts (unhappy → edit `answers.md` and re-run to redraft). Once they exist + conform, planning proceeds.
       - **`/plan`** (Spec Author proposes from the intake; the Architect sizes the proposals; then the **backlog gate** — the human PICKS which proposed features enter the sprint and commits them (`consort-sync-backlog --features …`, recorded in `requested.json`). That selection is the ONLY thing YOU (the coordinating session) do here — you do NOT author `feature-request.md` inline "as the PO". The drive then dispatches the **metered `product-owner` `author-requests` turn** (a real, tracked agent turn — `turn.usage` logs its tokens/cost, model settable via `roles.product-owner`) to author one `feature-request.md` per committed feature from the intake + proposals; a recorded seed that already exists + conforms is copied, not re-authored. The plan gate then locks the backlog).
...
KIT_REF="${KIT_REF:-v0.3.73}"   # stamped at release; == package.json version (enforced by tests/bdd/start-kit-pin.test.ts)
