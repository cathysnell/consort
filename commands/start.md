...
**Step 1 — offer the live dashboard as a WIZARD CHOICE, and START it yourself on yes.** Present a structured choice (AskUserQuestion) every time, whether or not they're in an editor — e.g. *"Open the live dashboard to watch this run?"* → **① Start it now** / **② Not now**. The human sees ONLY this clean choice and, on yes, the result — never a shell command.
- On **① Start it now:** run it YOURSELF via your Bash tool, **silently** — do NOT print the command, the log path, or `run-dashboard.sh` to the human, and never ask them to run it. Launch it DETACHED so it survives this turn and does not block it (internal-only, FOR YOU, do not echo: `setsid ./scripts/run-dashboard.sh >/tmp/consort-dashboard.log 2>&1 &`; or open it in a new tmux window / IDE terminal if you have one). It picks a free port, opens the browser radiator (roles, gates, per-turn transcripts, test counts) reading THIS project's `.consort/` off disk, and prints its URL to that log — read the log yourself and tell the human only *"Dashboard's live: <URL>"*. Then go to Step 2.
