...
init_esm_shims();
import { existsSync as existsSync3, readFileSync as readFileSync3, writeFileSync, readdirSync as readdirSync2 } from "fs";
...
// consort/optimize/optimize-apply.ts
var MANIFESTS_REL = join3("consort", "orchestrator", "steps", "manifests");
...
function applyAgentMdLevers(kitDir, plan) {
  ...
  for (const edit of plan.agentMdEdits) {
    const rel = join3("skills", "consort", "agents", `${edit.role}.md`);
    const path2 = join3(kitDir, rel);
    if (edit.kind === "replace-file") {
      writeFileSync(path2, edit.value);
      changed.add(`${edit.role}.md`);
      continue;
    }
    if (!existsSync3(path2)) throw new Error(`optimize-apply: cannot edit missing role definition ${rel}`);
    const md = readFileSync3(path2, "utf8");
...
---
`);
      writeFileSync(path2, nextFm + body);
      changed.add(`${edit.role}.md`);
    }
  }
  return [...changed];
...
function readRecordedCandidate(experimentsDir, handoff, candidateId) {
  const candDir = join4(experimentsDir, handoff, candidateId);
  if (!existsSync4(candDir)) throw new Error(`optimize-apply: no recorded candidate at ${candDir} (run the sweep first)`);
  const trials = readdirSync3(candDir).filter((d) => d.startsWith("trial-")).sort();
  if (trials.length === 0) throw new Error(`optimize-apply: no trials recorded under ${candDir}`);
  const file = join4(candDir, trials[0], "candidate.json");
  if (!existsSync4(file)) throw new Error(`optimize-apply: missing ${file}`);
...
async function main() {
  ...
  const experimentsDir = join4(projectDir, "experiments");
