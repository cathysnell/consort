...
// bin/consort/optimize-apply.cli.ts
...
// consort/optimize/optimize-apply.ts
init_cjs_shims();
...
// consort/optimize/optimize-apply.ts
var MANIFESTS_REL = (0, import_node_path2.join)("consort", "orchestrator", "steps", "manifests");
...
function applyAgentMdLevers(kitDir, plan) {
  ...
  for (const edit of plan.agentMdEdits) {
    const rel = (0, import_node_path2.join)("skills", "consort", "agents", `${edit.role}.md`);
    const path = (0, import_node_path2.join)(kitDir, rel);
    if (edit.kind === "replace-file") {
      (0, import_node_fs2.writeFileSync)(path, edit.value);
      changed.add(`${edit.role}.md`);
      continue;
    }
    if (!(0, import_node_fs2.existsSync)(path)) throw new Error(`optimize-apply: cannot edit missing role definition ${rel}`);
    const md = (0, import_node_fs2.readFileSync)(path, "utf8");
...
${edit.value}
`;
      (0, import_node_fs2.writeFileSync)(path, next);
      changed.add(`${edit.role}.md`);
...
---
`);
      (0, import_node_fs2.writeFileSync)(path, nextFm + body);
      changed.add(`${edit.role}.md`);
    }
  }
  return [...changed];
...
function readRecordedCandidate(experimentsDir, handoff, candidateId) {
  const candDir = (0, import_node_path3.join)(experimentsDir, handoff, candidateId);
  if (!(0, import_node_fs3.existsSync)(candDir)) throw new Error(`optimize-apply: no recorded candidate at ${candDir} (run the sweep first)`);
  const trials = (0, import_node_fs3.readdirSync)(candDir).filter((d) => d.startsWith("trial-")).sort();
  if (trials.length === 0) throw new Error(`optimize-apply: no trials recorded under ${candDir}`);
  const file = (0, import_node_path3.join)(candDir, trials[0], "candidate.json");
  if (!(0, import_node_fs3.existsSync)(file)) throw new Error(`optimize-apply: missing ${file}`);
...
function defaultKitDir() {
  return (0, import_node_path3.resolve)(new URL("../..", importMetaUrl).pathname);
}
async function main() {
  ...
  const projectDir = (0, import_node_path3.resolve)(args.projectDir);
  const kitDir = args.kitDir ? (0, import_node_path3.resolve)(args.kitDir) : defaultKitDir();
  const experimentsDir = (0, import_node_path3.join)(projectDir, "experiments");
