...
// bin/consort/sync-backlog.cli.ts
init_cjs_shims();
...
// consort/logging/agent-log.ts
function logFilePath(consortDir2) {
  return (0, import_path2.join)(consortDir2, "agent-log.jsonl");
}
...
function emitAgentLogEvent(input, opts = {}) {
  const consortDir2 = opts.consortDir ?? resolveConsortDir();
  const now = opts.now ?? (() => /* @__PURE__ */ new Date());
  const event = buildAgentLogEvent(input, now);
  const line = `${JSON.stringify(event)}
`;
  (0, import_fs2.appendFileSync)(logFilePath(consortDir2), line, "utf8");
...
// consort/gates/backlog-gate.ts
function approveBacklogGate(consortDir2, approver) {
  const dir = consortDir2 ?? resolveConsortDir();
  logGateApproved({ consortDir: dir, gate: "backlog", approver });
}

// bin/consort/sync-backlog.cli.ts
function parse(argv) {
  const out = { projectDir: process.cwd(), features: [], approver: "product-owner", json: false };
