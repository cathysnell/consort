...
- `.consort/features/<F>/architecture.json` (validated against its schema): the **NFRs** you propose (`nfrs[]`, each `applies_to` the feature or a story id, `hil_status: "proposed"`). NFRs live HERE, not on `feature-spec.json`/`story.json`.
  ...
  - **`nfrs` — ATOMIC per clause: ONE independently-testable obligation per entry, never a compound `fitness_function`.** Each `nfrs[]` entry's `fitness_function` is ONE thing the Test Strategist covers with ONE test. When a guarantee is inherently multi-part, emit SEPARATE `nfrs[]` entries — one per sub-clause, sharing the driving `brief_ref` — do NOT pack several checks into one entry's `fitness_function` string. The canonical offender is LAYERING: "dependencies point inward" is N directional import checks, and "only the repository touches the ORM session" is a whole-tree scan — split them into one NFR each: (a) `app/routes/` imports no ORM session; (b) ONLY `app/repositories/` touches the session, scanning the FULL `app/` tree MINUS `app/repositories/`; and one per inward direction your `layers[].may_import` implies — service ↛ routes, routes ↛ repositories (the boundary must not skip the service layer), repository ↛ service/routes, models ↛ any layer above it. A COMPOUND fitness_function forces the Test Strategist to infer N tests from one prose string and makes the reflect gate surface the uncovered sub-clauses ONE-PER-LAP (the piecemeal thrash: a full-design revise each lap that burns the reflect budget and, if it exceeds it, escalates to a human); ATOMIC per-clause NFRs give 1:1 test coverage so the design converges in a SINGLE reflect pass. The navigator's reflect already decomposes a compound clause as a backstop, but emitting atomic NFRs up front is the fix that avoids the loop.
